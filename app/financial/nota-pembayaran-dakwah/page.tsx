...
export default function NotaPembayaranDakwahPage() {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      setError('');
      const response = await fetch(`/api/financial/nota-pembayaran-dakwah?tahun=${tahun}`);
  ...
  const handleAutoGenerate = async () => {
    ...
    try {
      setGenerating(true);
      setError('');
      const response = await fetch('/api/financial/nota-pembayaran-dakwah', {
        method: 'POST',
  ...
  const handleSave = async () => {
    try {
      ...
      const url = '/api/financial/nota-pembayaran-dakwah';
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const response = await fetch(`/api/financial/nota-pembayaran-dakwah?id=${id}`, {
        method: 'DELETE',
      });
  ...
  return (
    <div className="container py-4">
      <div className="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-2">
        <h4 className="mb-0">Selenggara Nota Butiran Pembayaran Dakwah dan Pengimarahan</h4>
      ...
      {data && (
        <div className="card">
          <div className="card-header">
            <h6 className="mb-0">14. Dakwah Dan Pengimarahan (Sumbangan Am)</h6>
          </div>
      ...
      <div className="mt-3">
        <div className="alert alert-info">
          ...
          <strong>Nota:</strong> Klik butang "Jana Automatik" untuk menjana nilai dari transaksi yang telah dikategorikan sebagai "Dakwah dan Pengimarahan" dengan sub-kategori yang sesuai.
            ...
            <div className="modal-content">
              <div className="modal-header">
                <h5 className="modal-title">
                  {editingItem ? 'Edit' : 'Tambah'} Butiran Pembayaran Dakwah
                </h5>
