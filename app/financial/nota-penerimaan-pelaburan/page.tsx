...
interface NotaItem {
  id: number;
  jenis_pelaburan: string;
  institusi_cawangan: string | null;
...
export default function NotaPenerimaanPelaburanPage() {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      setError('');
      const response = await fetch(`/api/financial/nota-penerimaan-pelaburan?tahun=${tahun}`);
  ...
  const handleAutoGenerate = async () => {
    ...
    try {
      setGenerating(true);
      setError('');
      const response = await fetch('/api/financial/nota-penerimaan-pelaburan', {
        method: 'POST',
  ...
  const openAddModal = () => {
    setEditingItem(null);
    setFormData({ jenis_pelaburan: '', institusi_cawangan: '', jumlah_tahun_semasa: '', jumlah_tahun_sebelum: '' });
    setShowModal(true);
  };
  ...
  const openEditModal = (item: NotaItem) => {
    setEditingItem(item);
    setFormData({
      jenis_pelaburan: item.jenis_pelaburan,
      institusi_cawangan: item.institusi_cawangan || '',
  ...
  const handleSave = async () => {
    try {
      ...
      const url = '/api/financial/nota-penerimaan-pelaburan';
      const method = editingItem ? 'PUT' : 'POST';
      const body = {
        ...(editingItem ? { id: editingItem.id } : { tahun }),
        jenis_pelaburan: formData.jenis_pelaburan,
        institusi_cawangan: formData.institusi_cawangan || null,
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const response = await fetch(`/api/financial/nota-penerimaan-pelaburan?id=${id}`, {
        method: 'DELETE',
      });
  ...
  return (
    <div className="container py-4">
      <div className="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-2">
        <h4 className="mb-0">Selenggara Nota Butiran Penerimaan Pelaburan</h4>
      ...
      {data && (
        <div className="card">
          <div className="card-header d-flex justify-content-between align-items-center flex-wrap gap-2">
            <div>
              <h6 className="mb-0">8. Pelaburan (Hibah Pelaburan)</h6>
              <small className="text-muted">a. Pelaburan Islamic Simpanan Tetap</small>
            </div>
              ...
              <table className="table table-bordered table-hover mb-0">
                <thead className="table-light">
                  <tr>
                    ...
                    <th>Jenis Pelaburan</th>
                    <th>Institusi/Cawangan</th>
                ...
                <tbody>
                  {data.data.length === 0 ? (
                    ...
                    data.data.map((item, idx) => (
                      <tr key={item.id}>
                        <td>{idx + 1}</td>
                        <td>{item.jenis_pelaburan}</td>
                        <td>{item.institusi_cawangan || '-'}</td>
      ...
      <div className="mt-3">
        <div className="alert alert-info">
          ...
          <strong>Nota:</strong> Klik butang "Jana Automatik" untuk menjana nilai dari transaksi yang telah dikategorikan sebagai "Hibah Pelaburan" dengan maklumat jenis pelaburan dan institusi.
            ...
            <div className="modal-content">
              <div className="modal-header">
                <h5 className="modal-title">
                  {editingItem ? 'Edit' : 'Tambah'} Butiran Penerimaan Pelaburan
                </h5>
              ...
              <div className="modal-body">
                <div className="mb-3">
                  ...
                  <input
                    ...
                    value={formData.jenis_pelaburan}
                    onChange={(e) => setFormData({ ...formData, jenis_pelaburan: e.target.value })}
                    placeholder="cth: Pelaburan Mudharabah"
                    required
                  />
                </div>
                <div className="mb-3">
                  <label className="form-label">Institusi/Cawangan</label>
                  <input
                    type="text"
                    className="form-control"
                    value={formData.institusi_cawangan}
                    onChange={(e) => setFormData({ ...formData, institusi_cawangan: e.target.value })}
                    placeholder="cth: Bank Rakyat Seksyen 9 Shah Alam"
                  />
                </div>
                <div className="row">
