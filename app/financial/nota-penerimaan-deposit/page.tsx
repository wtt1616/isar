...
export default function NotaPenerimaanDepositPage() {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      setError('');
      const response = await fetch(`/api/financial/nota-penerimaan-deposit?tahun=${tahun}`);
  ...
  const handleAutoGenerate = async () => {
    ...
    try {
      setGenerating(true);
      setError('');
      const response = await fetch('/api/financial/nota-penerimaan-deposit', {
        method: 'POST',
  ...
  const openAddModal = () => {
    setEditingItem(null);
    setFormData({ deposit: '', tarikh: '', jumlah_tahun_semasa: '', jumlah_tahun_sebelum: '' });
    setShowModal(true);
  };
  ...
  const handleSave = async () => {
    try {
      ...
      const url = '/api/financial/nota-penerimaan-deposit';
      const method = editingItem ? 'PUT' : 'POST';
      const body = {
        ...(editingItem ? { id: editingItem.id } : { tahun }),
        deposit: formData.deposit,
        tarikh: formData.tarikh || null,
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const response = await fetch(`/api/financial/nota-penerimaan-deposit?id=${id}`, {
        method: 'DELETE',
      });
    ...
    <div className="container py-4">
      ...
      {data && (
        <div className="card">
          <div className="card-header d-flex justify-content-between align-items-center flex-wrap gap-2">
            <div>
              <h6 className="mb-0">9. Deposit Diterima/ Wang Cagaran</h6>
            </div>
      ...
      <div className="mt-3">
        <div className="alert alert-info">
          ...
          <strong>Nota:</strong> Klik butang "Jana Automatik" untuk menjana nilai dari transaksi yang telah dikategorikan sebagai "Deposit" dengan sub-kategori yang sesuai.
            ...
            <div className="modal-content">
              <div className="modal-header">
                <h5 className="modal-title">
                  {editingItem ? 'Edit' : 'Tambah'} Butiran Penerimaan Deposit
                </h5>
