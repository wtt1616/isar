...
// GET - Fetch all nota pembayaran dakwah for a specific year
export async function GET(request: NextRequest) {
  try {
    ...
    const [existingRecords] = await pool.query<NotaRow[]>(
      `SELECT * FROM nota_pembayaran_dakwah
       WHERE tahun = ?
       ORDER BY urutan, id`,
      [tahun]
    );
    ...
    console.error('Error fetching nota pembayaran dakwah:', error);
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (action === 'auto_generate') {
      ...
      // Get sums for current year grouped by sub_category_pembayaran for Dakwah dan Pengimarahan category
      const [currentYearSums] = await pool.query<TransactionSumRow[]>(
        `SELECT
         ...
         WHERE category_pembayaran = 'Dakwah dan Pengimarahan'
      ...
      await pool.query(
        'DELETE FROM nota_pembayaran_dakwah WHERE tahun = ? AND auto_generated = TRUE',
        [targetYear]
      );
      ...
      for (const row of currentYearSums) {
        ...
        await pool.query(
          `INSERT INTO nota_pembayaran_dakwah
    ...
    const [maxUrutan] = await pool.query<RowDataPacket[]>(
      'SELECT MAX(urutan) as max_urutan FROM nota_pembayaran_dakwah WHERE tahun = ?',
      [tahun]
    );
    ...
    console.error('Error creating/auto-generating nota pembayaran dakwah:', error);
...
export async function PUT(request: NextRequest) {
  try {
    ...
    console.error('Error updating nota pembayaran dakwah:', error);
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    await pool.query('DELETE FROM nota_pembayaran_dakwah WHERE id = ?', [id]);

    return NextResponse.json({ success: true });
  } catch (error) {
    console.error('Error deleting nota pembayaran dakwah:', error);
