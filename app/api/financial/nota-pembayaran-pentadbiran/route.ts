...
export async function POST(request: NextRequest) {
  try {
    ...
    if (action === 'auto_generate') {
      ...
      // Get sums for current year grouped by sub_category_pembayaran for Pentadbiran category
      // sub_category_pembayaran will be mapped to sub_kategori1, and we need another field for sub_kategori2
      const [currentYearSums] = await pool.query<TransactionSumRow[]>(
        `SELECT
           COALESCE(sub_category_pembayaran, 'Lain-lain') as sub_category1,
