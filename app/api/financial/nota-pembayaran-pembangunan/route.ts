...
// GET - Fetch all nota pembayaran pembangunan for a specific year
export async function GET(request: NextRequest) {
  try {
    ...
    const [existingRecords] = await pool.query<NotaRow[]>(
      `SELECT * FROM nota_pembayaran_pembangunan
       WHERE tahun = ?
       ORDER BY urutan, id`,
      [tahun]
    );
    ...
    console.error('Error fetching nota pembayaran pembangunan:', error);
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (action === 'auto_generate') {
      ...
      // Get sums for current year grouped by sub_category_pembayaran for Pembangunan dan Penyelenggaraan category
      const [currentYearSums] = await pool.query<TransactionSumRow[]>(
        `SELECT
           COALESCE(sub_category_pembayaran, 'Lain-lain') as perkara,
           SUM(debit_amount) as total
         FROM financial_transactions
         WHERE category_pembayaran = 'Pembangunan dan Penyelenggaraan'
         AND YEAR(transaction_date) = ?
         GROUP BY sub_category_pembayaran`,
        [targetYear]
      );
      ...
      await pool.query(
        'DELETE FROM nota_pembayaran_pembangunan WHERE tahun = ? AND auto_generated = TRUE',
        [targetYear]
      );
      ...
      for (const row of currentYearSums) {
        ...
        await pool.query(
          `INSERT INTO nota_pembayaran_pembangunan
    ...
    const [maxUrutan] = await pool.query<RowDataPacket[]>(
      'SELECT MAX(urutan) as max_urutan FROM nota_pembayaran_pembangunan WHERE tahun = ?',
      [tahun]
    );
    ...
    console.error('Error creating/auto-generating nota pembayaran pembangunan:', error);
...
export async function PUT(request: NextRequest) {
  try {
    ...
    await pool.query(
      `UPDATE nota_pembayaran_pembangunan
       SET perkara = ?,
           jumlah_tahun_semasa = ?,
    ...
    console.error('Error updating nota pembayaran pembangunan:', error);
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    await pool.query('DELETE FROM nota_pembayaran_pembangunan WHERE id = ?', [id]);

    return NextResponse.json({ success: true });
  } catch (error) {
    console.error('Error deleting nota pembayaran pembangunan:', error);
